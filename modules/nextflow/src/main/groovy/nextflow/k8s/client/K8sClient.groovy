...
class K8sClient {
     ...
     *
     */
    Map podState( String podName, boolean initContainer = false ) {
        assert podName

        final K8sResponseJson resp = podStatus0(podName)
        final status = resp.status as Map
        final containerStatuses = initContainer ?
                status?.initContainerStatuses as List<Map> :
                status?.containerStatuses as List<Map>
        ...
        if( containerStatuses?.size()>0 ) {
            ...
            if( !initContainer && (!container.name || !podName.startsWith(container.name.toString())) || initContainer && container.name != 'setup-environment' )
